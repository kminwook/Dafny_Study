...
predicate listIsAllEven(l : ilist){
   match l
   case Nil => true
   case Cons(x,xs) => if x%2 != 0 then false
   else listIsAllEven(xs)
}
/* YOUR WORK GOES HERE */
...
function filterEvens(l:ilist) : ilist{
   match l
   case Nil => Nil
   case Cons(x,xs) => if x%2 == 0 then append(Cons(x,Nil),filterEvens(xs))
   else filterEvens(xs)
}
/* YOUR WORK GOES HERE */
...
ensures filterEvens(Cons(2,Cons(3,Cons(4,Nil)))) == Cons(2,Cons(4,Nil)) {}
/* YOUR WORK GOES HERE */
...
ensures length(l) >= length(filterEvens(l)){}
/* YOUR WORK GOES HERE */
