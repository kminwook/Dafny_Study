// Dafny program update-dates.dfy compiled into C#
...
// Command-line arguments: run --no-verify c:\Users\김민욱\Desktop\dafny programs\lecture03\update-dates.dfy
// update-dates.dfy
...
namespace _module {
  ...
  public partial class __default {
    public static _Idate makeFirstOfJan(_Idate d) {
      _Idate _0_dt__update__tmp_h0 = d;
      BigInteger _1_dt__update_hmonth_h0 = BigInteger.One;
      BigInteger _2_dt__update_hday_h0 = BigInteger.One;
      return _module.date.create(_2_dt__update_hday_h0, _1_dt__update_hmonth_h0, (_0_dt__update__tmp_h0).dtor_year);
    }
    public static _Idate nextday(_Idate d) {
      if (((d).dtor_day) == (new BigInteger(31))) {
        if (((d).dtor_month) == (new BigInteger(12))) {
          _Idate _0_dt__update__tmp_h0 = d;
          BigInteger _1_dt__update_hyear_h0 = ((d).dtor_year) + (BigInteger.One);
          BigInteger _2_dt__update_hmonth_h0 = BigInteger.One;
          BigInteger _3_dt__update_hday_h0 = BigInteger.One;
          return _module.date.create(_3_dt__update_hday_h0, _2_dt__update_hmonth_h0, _1_dt__update_hyear_h0);
        } else {
          _Idate _4_dt__update__tmp_h1 = d;
          BigInteger _5_dt__update_hmonth_h1 = ((d).dtor_month) + (BigInteger.One);
          BigInteger _6_dt__update_hday_h1 = BigInteger.One;
          return _module.date.create(_6_dt__update_hday_h1, _5_dt__update_hmonth_h1, (_4_dt__update__tmp_h1).dtor_year);
        }
      } else {
        _Idate _7_dt__update__tmp_h2 = d;
        BigInteger _8_dt__update_hday_h2 = ((d).dtor_day) + (BigInteger.One);
        return _module.date.create(_8_dt__update_hday_h2, (_7_dt__update__tmp_h2).dtor_month, (_7_dt__update__tmp_h2).dtor_year);
      }
    }
  }

  public interface _Idate {
    bool is_Date { get; }
    BigInteger dtor_day { get; }
    BigInteger dtor_month { get; }
    BigInteger dtor_year { get; }
    _Idate DowncastClone();
  }
  public class date : _Idate {
    public readonly BigInteger _day;
    public readonly BigInteger _month;
    public readonly BigInteger _year;
    public date(BigInteger day, BigInteger month, BigInteger year) {
      this._day = day;
      this._month = month;
      this._year = year;
    }
    public _Idate DowncastClone() {
      if (this is _Idate dt) { return dt; }
      return new date(_day, _month, _year);
    }
    public override bool Equals(object other) {
      var oth = other as date;
      return oth != null && this._day == oth._day && this._month == oth._month && this._year == oth._year;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._day));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._month));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._year));
      return (int) hash;
    }
    public override string ToString() {
      string s = "date.Date";
      s += "(";
      s += Dafny.Helpers.ToString(this._day);
      s += ", ";
      s += Dafny.Helpers.ToString(this._month);
      s += ", ";
      s += Dafny.Helpers.ToString(this._year);
      s += ")";
      return s;
    }
    private static readonly _Idate theDefault = create(BigInteger.Zero, BigInteger.Zero, BigInteger.Zero);
    public static _Idate Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_Idate> _TYPE = new Dafny.TypeDescriptor<_Idate>(date.Default());
    public static Dafny.TypeDescriptor<_Idate> _TypeDescriptor() {
      return _TYPE;
    }
    public static _Idate create(BigInteger day, BigInteger month, BigInteger year) {
      return new date(day, month, year);
    }
    public static _Idate create_Date(BigInteger day, BigInteger month, BigInteger year) {
      return create(day, month, year);
    }
    public bool is_Date { get { return true; } }
    public BigInteger dtor_day {
      get {
        return this._day;
      }
    }
    public BigInteger dtor_month {
      get {
        return this._month;
      }
    }
    public BigInteger dtor_year {
      get {
        return this._year;
      }
    }
